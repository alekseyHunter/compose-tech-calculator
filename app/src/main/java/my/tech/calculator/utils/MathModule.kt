...
class MathModule(expression: String) {
    ...
    class LexicalAnalyzer(expression: String) {
        private val lexemes: List<Lexeme>
        private var position: Int

        init {
            lexemes = lexAnalyze(expression)
            position = 0
        }
    ...
    fun calculate(): Double {
        val lexeme = this.lexicalAnalyzer.next()
        return if (lexeme.type == LexemeType.EOF) {
            0.0
        } else {
            this.lexicalAnalyzer.back()
            plusminus(this.lexicalAnalyzer)
        }
    }

    private fun plusminus(lexemes: LexicalAnalyzer): Double {
    ...
    private fun muldiv(lexemes: LexicalAnalyzer): Double {
    ...
    private fun factor(lexemes: LexicalAnalyzer): Double {
